...
float FCriticalHit::CalculateValue(const uint16 Level)
{
	...
	const float PrevDecaLevel = 10 * FMathf::Floor(UUtilityFunctionLibrary::RoundToDecimals((Level + 0.1f)/10.0f, 3));
	const float NextDecaLevel = 10 * FMathf::Ceil(UUtilityFunctionLibrary::RoundToDecimals((Level + 0.1f)/10.0f, 3));
