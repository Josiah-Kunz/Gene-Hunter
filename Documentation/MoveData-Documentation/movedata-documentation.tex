...
\begin{tldr}
	\begin{itemize}
		\item{\code{MoveData} holds static data about a Move. You can think of it like a Bulbapedia page on the Move.}
		\item{\code{MoveData} should not be confused with \code{MoveInstance} (which tracks things like cooldown).}
...
\begin{itemize}
	\bitem{Actors to spawn}{An array of \code{UActorSpawnScheme}s, which are \code{UObjects}. Like most \code{UObject}s, you can create one in-editor by making \code{UActorSpawnScheme} (or a derived class) the base class. Minimally, this includes an array of Actors to spawn. However, you can inherit and customize to suit your needs. For example, maybe you want a chance to spawn 1 apple (50\%), 2 bananas (30\%), or nothing (20\%).}
	...
	\bitem{Usability scheme}{This determines under what conditions a Move is usable. This has nothing to do with cooldown, but rather extra conditionals. For example, perhaps an Execute skill can only be used when the foe has <20\% health. If the usability scheme is null, the Move is usable under any conditions.}
\end{itemize}
...
\sect{Damage Calculation Formula}\label{sec:damage-calc-formula}
