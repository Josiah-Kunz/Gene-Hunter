...
\begin{longtblr}[
	...
	%----------------------
	\hline
	\SetCell[c=3]{l}{\color{ICBlue}\textbf{GetBaseExpYield}}\\
	\hline
	&&\textit{Note:}\\
	%----------------------	
	...
	%----------------------
	\hline
	\SetCell[c=3]{l}{\color{ICBlue}\textbf{SetBaseExpYield}}\\
	\hline
	&& \textit{Note}\\
	%----------------------	
	...
	%----------------------
	\hline
	\SetCell[c=3]{l}{\color{ICBlue}\textbf{SetCXP}}\\
	\hline
	&& \textit{Note}\\
	%----------------------	
...
\begin{enumerate}
	...
	\item{\textbf{Create complementary Outlet.} If you create a ``Before'' Outlet, you should probably also create an ``After'' Outlet. The easiest way is copy + paste, then replace ``Before'' with ``After''. You might also need to modify the delegate signature (e.g., reference ``\code{\&}'' to \code{const}).
		\begin{itemize}
